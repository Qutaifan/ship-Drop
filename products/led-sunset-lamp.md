# Product Validation Template

## Product
- Name: LED Sunset Projection Lamp
- Source/supplier candidate(s):
- Retail price target (VAT-inclusive, as the customer pays): €29.90
- VAT rate (destination market, e.g. 0.19 for DE): 0.19
- Product cost: €4.50
- Shipping cost: €2.50
- Import duty per unit (EU: €3 flat per customs item since 2026-07-01; 0 if fulfilled from an EU warehouse): €0.00

## 6-Criteria Screen (PROTOCOL-01 / AGENTS.md §4A)
1. Wow Factor (sub-3s emotional hook?):
2. Problem Solving (what friction does it resolve?):
3. Visual Appeal (communicable in silent 9:16 video?):
4. Healthy Margins (€15–30/sale target; reject <€20 retail):
5. Low Return Potential (simple/durable mechanics?):
6. Low Local Retail Availability:

## Competitor Check
- Competitors running ads (need 5–10):
- Ads active 30+ days (need ≥3, proves sustained profitability):

## Multi-pass Demand Screen
- Query: "LED sunset projection lamp"
- Sampled videos: 25
- Median views: 249
- Skeptic ratio: 32%
- Stability: STABLE
- Criterion 3 (Proof Burden): PASS
- Demand Floor (Median Views ≥2,500): FAIL (near-zero content demand)
- Pre-Screen Verdict: FAIL

## True Margin Matrix
`Net Margin = (Retail / (1 + VAT)) − (Product Cost + Shipping + Import Duty) − (0.03 × Retail)`
- Retail price target (VAT-inclusive): €29.90
- VAT rate: 0.19
- Product cost: €4.50
- Shipping cost: €2.50
- Import duty per unit: €0.00
- COGS (Cost + Shipping + Duty): €7.00
- Ex-VAT Revenue: €25.13
- 3% Payment fee: €0.90
- Net Margin: €14.23
- 3x COGS Gate (Need ≥ €21.00): FAIL
- Floor Gate (Need > €15.00): PASS
- Retail Floor Gate (Need ≥ €20.00): PASS
- Margin Matrix Verdict: FAIL

### Buying Constraint — Maximum Landed Cost Reference
Solving both gates for maximum landed cost at 19% VAT:

| Gross retail | Max landed cost | Net margin at that cost |
|---|---|---|
| €24.90 | €5.04 | €15.13 |
| €29.90 | €6.06 | €18.17 |
| €34.90 | €7.07 | €21.21 |
| €39.90 | €8.08 | €24.25 |
| €44.90 | €9.10 | €27.29 |
| €49.90 | €10.11 | €30.33 |

**Rule: landed cost must be ≤ 20.3% of the VAT-inclusive retail price.**

## Regulatory Check
- De Minimis / sourcing region:
- EU AI Act disclosure needed (AI imagery/support)?:
- FTC dynamic pricing rule-based (not user-tracked)?:

## Verdict
FAIL — demand floor failed (249 median views) and 3x COGS gate not met. Kill candidate unless stronger external demand evidence appears.
